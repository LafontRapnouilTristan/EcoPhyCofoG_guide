...
[EPSON's V800](Machine/EPSON_V800/info.) scanners are the ones used as this document is being written. The scanner model isn't important but we recommend to use scanners with a transparent (double-lamp) option. This will allow cleaner root scans for complex root systems.
And the scanning software is [ViewScan](Software/Viewscan/test.txt)
...
knitr::include_graphics('Traits/RootMorpho/Figures/')
```
...
knitr::include_graphics('Traits/RootMorpho/Figures/bromeroot.tif')
```
...
knitr::include_graphics('Traits/RootMorpho/Figures/scan_transp.png')
```
...
knitr::include_graphics('Traits/RootMorpho/Figures/scan_exemple.tif')
```
...
Detailed `XnConvert` tutorial is available [HERE](Software/XnConvert/).

```{r stencil, echo=FALSE, out.width="50%", fig.cap="Stencil used for inwater root scans"}
knitr::include_graphics('Traits/RootMorpho/Figures/squre_stencil.jpg')
```
...
You can practice with scans available [here](Traits/RootMorpho/Data/)
...
Please, when you create a `.CAL` at a previously not used resolution, store a copy of the calibration file [here](Software/Winrhizo/) so that your work helps your successors!!
